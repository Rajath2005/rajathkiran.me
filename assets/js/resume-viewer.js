...
(function(){
  ...
  let pdfDoc = null; let pageNum = 1; let scale = 1.0; let rotation = 0;
  ...
  const queueRender = (num) => { if(!pdfDoc) return; if(num<1) num=1; if(num>pdfDoc.numPages) num=pdfDoc.numPages; pageNum = num; renderPage(pageNum); };

  function snapToPreset(t){ let nearest = presets.reduce((a,b)=> Math.abs(b-t)<Math.abs(a-t)?b:a); scale = nearest; queueRender(pageNum); }
  ...
  const load = async () => {
    try{
      ...
      pageNum = 1; scale = 1.0; rotation = 0; renderPage(pageNum);
  ...
  document.getElementById('zoomIn').addEventListener('click', ()=>{ const next = presets.find(p=>p>scale) || presets[presets.length-1]; scale = next; queueRender(pageNum); });
  document.getElementById('zoomOut').addEventListener('click', ()=>{ const prev = [...presets].reverse().find(p=>p<scale) || presets[0]; scale = prev; queueRender(pageNum); });

  document.getElementById('fitBtn').addEventListener('click', ()=>{
    // fit to container width
    const container = document.getElementById('canvasContainer');
    const pageSample = pdfDoc.getPage(pageNum).then(page=>{
      const vp = page.getViewport({scale:1, rotation});
      const containerWidth = container.clientWidth - 24; // padding
      const newScale = containerWidth / vp.width;
      scale = Math.max(0.5, Math.min(newScale, 3)); queueRender(pageNum);
    });
  });
  ...
  container.addEventListener('wheel', (e)=>{
    ...
    if(e.deltaY < 0) { const next = presets.find(p=>p>scale) || presets[presets.length-1]; scale = next; }
    else { const prev = [...presets].reverse().find(p=>p<scale) || presets[0]; scale = prev; }
    queueRender(pageNum);
  ...
  container.addEventListener('touchstart', (e)=>{
    if(e.touches && e.touches.length===2){ pinch.active=true; const dx = e.touches[0].clientX-e.touches[1].clientX; const dy = e.touches[0].clientY-e.touches[1].clientY; pinch.startDist = Math.hypot(dx,dy); pinch.startScale = scale; }
  }, {passive:true});
  container.addEventListener('touchmove', (e)=>{
    if(pinch.active && e.touches.length===2){ const dx = e.touches[0].clientX-e.touches[1].clientX; const dy = e.touches[0].clientY-e.touches[1].clientY; const dist = Math.hypot(dx,dy); const ratio = dist / pinch.startDist; scale = Math.max(0.5, Math.min(pinch.startScale * ratio, 3)); renderPage(pageNum); }
  }, {passive:true});
  container.addEventListener('touchend', (e)=>{ if(pinch.active){ pinch.active=false; snapToPreset(scale); } }, {passive:true});

  // keyboard
  window.addEventListener('keydown',(e)=>{
    if(e.key==='ArrowLeft') queueRender(pageNum-1);
    if(e.key==='ArrowRight') queueRender(pageNum+1);
    if(e.key==='+'||e.key==='=') { const next = presets.find(p=>p>scale) || presets[presets.length-1]; scale = next; queueRender(pageNum); }
    if(e.key==='-') { const prev = [...presets].reverse().find(p=>p<scale) || presets[0]; scale = prev; queueRender(pageNum); }
